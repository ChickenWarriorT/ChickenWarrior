...
namespace ExcelTool
{
    ...
    public class ExcelEditorWindow : EditorWindow
    {
        ...
        public static void PrOnReloadToCreate()
        {
            ...
            {
                //Debug.Log($"类型 ： {key}|{dicValues[key].FieldType}|{dic[key][0].GetType()}");
        ...
        private static void PrReadExcel()
        {
            ...
            foreach (var info in GetFileInfos(clsSetting.strExcelPath))
            {
                ...
                foreach (var worksheet in excelPackage.Workbook.Worksheets)
                {
                    ...
                    PrGetDicValues(info.Name.Substring(0, info.Name.LastIndexOf('.')), worksheet);
                }
            }
            //Enum先生成脚本
            foreach (var info in clsInfo.infos)
        ...
        private static void PrGetDicValues(string fileName, ExcelWorksheet worksheet)
        {
            ...
            {
                Debug.Log("Worksheet is empty");
                return; // return early
            }
            //Debug.Log("fileName : " + worksheet.Dimension.End.Column);
            clsInfo.infos.Add(new SheetInfo(fileName + "_" + worksheet.Name));
                        ...
                        try
                        {
                            switch (type)
                            {
                                ...
                                case "float":
                                    ...
                                    break;
                                case "list-int":
                                    List<int> intList = new List<int>(value.Split(';').Select(int.Parse).ToList());
                                    Debug.Log(intList[0] + ":" + intList[1]);
                                    dicValue[name].SetValue(arrClones.GetValue(j), intList);
                                    break;
